...
Either backend can land the extracted files at different nesting depths depending on version/cache state, so the downstream code never assumes a fixed path: `find_images_dir()` and `_find_captions_file()` walk the downloaded tree and locate the `Images/` folder and `captions.txt` programmatically. `verify_dataset()` then sanity-checks that ~8,000 images were actually retrieved before anything downstream runs.
...
`scripts/evaluate.py` also renders `artifacts/results/qualitative_examples.png` — a contact sheet pairing each sampled test photo with its generated caption and all 5 human references:

![qualitative examples](artifacts/results/qualitative_examples.png)
