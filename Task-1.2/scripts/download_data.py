...
def main() -> None:
    ...
    verify_dataset(paths, expected_min_images=cfg.dataset.expected_min_images)
    stable_paths = copy_into_processed(paths, processed_dir)
    from captioner.data.download import write_dataset_manifest
    # Use stable_paths (not the pre-copy `paths`) so the manifest points at
    # the captions file actually copied into data/processed -- the one that
    # survives even if the original Kaggle cache / --local-dir source is
    # later cleared or moved.
    write_dataset_manifest(stable_paths, processed_dir)
